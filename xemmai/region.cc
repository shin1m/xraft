...
t_type* t_type_of<xraft::t_region>::f_derive(t_object* a_this)
{
	return new t_type_of(v_module, a_this);
}

void t_type_of<xraft::t_region>::f_finalize(t_object* a_this)
{
	delete &f_as<xraft::t_region&>(a_this);
}

t_scoped t_type_of<xraft::t_region>::f_construct(xemmai::t_object* a_class, t_stacked* a_stack, size_t a_n)
{
	return t_construct<>::t_bind<xraft::t_region>::f_do(a_class, a_stack, a_n);
}
