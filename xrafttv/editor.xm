...
Editor = Class(xraft.Frame) :: @
	...
	key_binds = {
		xraft.Key.LEFT: @ $_view.caret_move_text_backward(1
		xraft.Key.RIGHT: @ $_view.caret_move_text_forward(1
		xraft.Key.UP: @ $_view.caret_move_line_backward(1
		xraft.Key.DOWN: @ $_view.caret_move_line_forward(1
		xraft.Key.RETURN: @ $insert("\n"
		xraft.Key.BACK_SPACE: @ $backspace(
		xraft.Key.DELETE: @ $delete(
		xraft.Key.F1: @ for c = 0x20; c <= 0x7e; c = c + 1
			$insert(String.from_code(c
		xraft.Key.F2: @
		...
		xraft.Key.F3: @
			n = $_model.text_size(
			for i = 0; i < n; i = i + 10
				system.out.write_line($_model.slice(i, 10
	...
	$on_input_compose = @
		if $_composing_size <= 0
			$_composing_begin = $_view.caret_text(
		...
		while i < attributes.size()
			a = attributes[i]
			j = i + 1
			while j < attributes.size() && attributes[j] == a
				j = j + 1
			segments.push('(j - i, $_attribute_compose[a]
			if (a & xraft.InputAttribute.REVERSE) != 0
				caret = j
			i = j
	...
	$on_button_press = @(modifier, button, x, y) button == xraft.Button.BUTTON3 && $on_close(
	...
	$position__ = @(position)
		...
		if position + extent.height() > limit
			position = limit - extent.height()
		if position < 0
			position = 0
		position == $_position && return
		$scroll(0, 0, extent.width(), extent.height(), 0, $_position - position
		$_position = position
	$scroll_viewable = @(y, height)
		extent = $geometry(
		position = $_position
		if position + extent.height() < y + height
			position = y + height - extent.height()
		if position > y
			position = y
		$position__(position
