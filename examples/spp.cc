...
struct t_function : t_runnable
{
	std::function<void()> v_function;

	t_function(std::function<void()>&& a_function) : v_function(std::move(a_function))
	{
	}
...
{
	if (dbus_message_get_type(a_result) == DBUS_MESSAGE_TYPE_ERROR) {
		const char* p;
...
std::string f_path(const dbus::t_message& a_message, const char* a_device, const char* a_uuid)
{
	DBusMessageIter i;
		...
		for (t_container_parser interfaces(path, DBUS_TYPE_ARRAY); dbus_message_iter_get_arg_type(interfaces) != DBUS_TYPE_INVALID; dbus_message_iter_next(interfaces)) {
			...
			for (t_container_parser properties(interface, DBUS_TYPE_ARRAY); dbus_message_iter_get_arg_type(properties) != DBUS_TYPE_INVALID; dbus_message_iter_next(properties)) {
				...
				if (std::strcmp(property.v_key, "Address") == 0) {
					...
					const char* address;
					value >> address;
					address_matched = std::strcmp(address, a_device) == 0;
			...
			if (address_matched && uuid_matched) return path.v_key;
		}
	}
	return {};
}
...
class t_main
{
	...
	std::string v_device;
	int v_fd = -1;
	t_function v_local{[this]
	{
		f_copy(0, v_fd);
	}};
	t_function v_remote{[this]
	{
		f_copy(v_fd, 1);
	}};
	...
	{
		if (v_fd >= 0) {
			f_application()->f_poll(v_fd, nullptr, nullptr);
			close(v_fd);
		}
		v_fd = a_fd;
		if (v_fd >= 0) f_application()->f_poll_reader(v_fd, &v_remote);
	}
...
public:
	t_main(const std::string& a_device)
	{
		...
		f_check(result);
		v_device = f_path(result, a_device.c_str(), v_uuid_spp);
		...
		}
		{
			...
			dbus::t_reply(p, v_connection)([this](auto a_result)
			{
				...
				f_application()->f_poll_reader(0, &v_local);
			});
		}
	}
	~t_main()
